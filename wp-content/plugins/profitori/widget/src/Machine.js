...
export default class Machine {
  ...
  addMenuItem(aName) {
    Menu.add({specname: global.currentSpecname, caption: aName})
  }
  ...
  specnameToAttributes(aSpecname) {
    ...
    if ( ! isExt ) {
      ...
      if ( idx < 0 ) throw(new Error("Spec " + sn + " must be included in index.js"))
  ...
  maint(aName, aParms) {
    ...
    if ( this.attributes._identified ) {
  ...
  method(aDatatype, aName, aFn) {
    ...
    if ( ! m ) {
      ...
      if ( (a.componentName !== 'List') && (a.componentName !== 'Maint') ) this.err("method target must be a datatype, list or maint")
  ...
  enhance(aDatatype, aName, aFn) {
    ...
    let enhanceFunction = function(obj, aName, aFn) {
      let originalFunction = obj[aName]; if ( ! originalFunction ) this.err('enhance called on non-existent function ' + aName)
  ...
  visibleWhen(aName, aVal) {
    let manifest = this.nameToManifest(aName)
    if ( manifest ) {
      manifest.visibleWhen = aVal
      return
    }
    ...
    let f = this.nameToField(aName) //; if ( ! f ) this.err("visibleWhen: unknown field: " + aName)
    if ( f )
      f.visibleWhen = aVal
    else {
  ...
  calculate(aFieldName, aVal) {
    let f = this.nameToField(aFieldName); if ( ! f ) this.err("calculate: unknown field: " + aFieldName)
    //if ( (! global.runningInsideWordpress) && (! global.isAsync(aVal)) ) this.err("calculate function must be async")
    f.calculate = aVal
    let m = f.toMold()
    if ( m )
  ...
  afterTrash(aName, aFn) {
    let a = this.attributes
    if ( a.componentName !== 'List' ) 
      this.err("afterTrash target must be a list")
  ...
  field(aName, aParms) {
    ...
    this.useParm(aParms, "caption", f, null, {ignore: inUIButDoingMolds})
    ...
    if ( f.beforeField )
      this.placeFieldBefore(f, f.beforeField, fields)
    if ( f.afterField )
      this.placeFieldAfter(f, f.afterField, fields)
    if ( f.tickbox )
      f.yesOrNo = true
  ...
  placeFieldBefore(field, beforeFieldName, fields) {
  ...
  placeFieldAfter(field, afterFieldName, fields) {
    let fieldIdx = this.fieldNameToIndex(field.name, fields)
  ...
  title(aTitle, aParms) {
    ...
    if ( ! headerOnly )
      global.stMenu.setItemCaption(global.currentSpecname, aTitle)
  }
