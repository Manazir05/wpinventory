...
'Source'.data(async function() {
  ...
  let addWCProductMetakeys = async () => {
    addSource({description: "WC Product.name", invLev: true, allowEdit: false})
    let metakeys = await 'metakey'.bring()
    metakeys.forAll(metakey => {
      addSource({description: "WC Product." + metakey.name, invLev: true, allowEdit: false})
    })
  }
  ...
  let addWCProductAttributes = async () => {
    ...
    attributes.forAll(attribute => {
      addSource({description: "WC Product Attribute." + attribute.name, invLev: true, allowEdit: false})
    })
  }
