...
'Generate Stocktake'.availableWhen((cast, list) => {
  return (! list.situation()._doingBulkLabels) && (! list.situation()._doingBulkTrash) && (! list.situation()._doingGenerateStocktake)
})
...
'InventoryLevels'.modifyFields(async (grid, fields) => {
  ...
  fields.splice(6, fields.length - 1) // Always include the product, sku, the Inventory reference and the 3 tickboxes
