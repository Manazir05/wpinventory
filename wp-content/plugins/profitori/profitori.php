...
function prfi_stale_datatype_ex($datatype, $stale_me, $excision_object_id) {
  $include_me = $stale_me;
...
function prfi_available_payment_gateways($gateways) {
  ...
  if ( ! prfi_customer_has_sufficient_credit() ) 
...
function prfi_on_init() {
  ...
  add_action('woocommerce_after_cart', 'prfi_after_cart', 10, 1);
}

function prfi_emit_impost_javascript() {
  global $woocommerce;
...
function prfi_after_cart( $wccm_after_checkout ) {
  if ( prfi_conf_val('stImp') !== 'Yes' ) 
    return;
  prfi_emit_impost_javascript();
} 
...
function prfi_get_shipment_lines() {
  ...
  $sql = $wpdb->prepare("
      ...
      GROUP BY soShipmentLine.ID
      ORDER BY soShipmentLine.ID ASC;
      ",
      get_current_user_id()
  );
  $res = $wpdb->get_results($sql, ARRAY_A);
...
function prfi_emit_shipment_td($field, $shipment_line) {
  ?>
    <td>
      <?php
        if ( isset($shipment_line[$field]) )
          echo $shipment_line[$field];
      ?>
    </td>
  <?php
}
...
function prfi_shipment_line_to_attachments($shipment_line) {
  ...
  $sql = $wpdb->prepare("
      ...
      LEFT JOIN 
        $wpdb->postmeta _fileName ON attachment.ID = _fileName.post_id AND 
          _fileName.meta_key = 'stocktend_fileName'
      LEFT JOIN 
...
function prfi_maybe_add_stock_clue($point, $castId, $fieldName, $oldVal, $newVal, $reference='', $datatype='products') {
  ...
  prfi_set_meta($id, "stocktend_time", date('H:i:s'));
...
function prfi_create_morsel($aType, $aOrderItemId, $aProductId, $aQuantity, $aAmount, $aOrderId=null) {
...
class PRFI_Captain extends WP_REST_Posts_Controller {
  ...
  function save_request_as_posts($aReq) {
    ...
    foreach ($objs as $obj) {
      ...
      if ( $obj->_datatype === "products" )  {
        $post = $this->save_object_as_product($obj);
      } else {
        $ghost = $this->object_to_ghost($obj, null);
        $post = $this->save_ghost_as_post($ghost);
        prfi_stale_datatype($obj->_datatype);
      }
      if ( isset($obj->_datatype) && ($obj->_datatype === "Inventory") )
  ...
  function do_save_object_as_product($aObj) {
    $id = $aObj->id;
    ...
    if ( isset($aObj->_stock) ) {
      $oldVal = $prod->get_stock_quantity();
  ...
  function save_object_as_product($aObj) {
    global $prfi_am_updating_product_directly;
    if ( isset($aObj->__submethod) && ($aObj->__submethod === 'untrash') ) {
      ...
      $id = $this->do_save_object_as_product($aObj);
...
*/
  ...
  function save_ghost_as_post($aGhost) {
    if ( $aGhost->ID === null ) {
      ...
      //tc(20);
      $id = $this->update_post($aGhost); prfi_chk($id);
      //tc(21);
    }
    //tc(22);
    $post = get_post($id);
  ...
  function update_post_meta($aGhost) {
    global $wpdb;
    while ( true ) {
      $meta_value = current($aGhost->meta_input);
      if ( $meta_value === FALSE ) break;
  ...
  function field_names_to_cols($aNames, $aNative=false) {
    global $wpdb;
    global $prfi_subselects;
    $res = '';
    $idx = 0;
    ...
    foreach ( $aNames as $name ) {
      ...
      if ( $prfi_subselects || ($idx >= 58) ) // joins are limited to 61
  ...
  function row_to_object($aRow, $aDatatype) {
    $obj = array();
  ...
  function get_shipping_method_rows($aId) {
    ...
    $rows = array();
      ...
      $shipping_methods = $zone['shipping_methods'];
        ...
        $uniqueName = $zone_name . ': ' . $method_title;
