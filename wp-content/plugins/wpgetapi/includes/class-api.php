...
class WpGetApi_Api {
    ...
    public $body_json_encode = ''; // the body_json_encode
    ...
    public function init() {
        ...
        foreach ( $this->api['endpoints'] as $index => $endpoint ) {
            ...
            if( $this->endpoint_id == $endpoint['id'] ) {
                ...
                $this->body_json_encode = isset( $endpoint['body_json_encode'] ) && ! empty( $endpoint['body_json_encode'] ) ? sanitize_text_field( $endpoint['body_json_encode'] ) : true;
        ...
        if( is_array( $parameters ) ) {
            foreach ( $parameters as $key => $parameter ) {
                if( ! empty( $parameter['name'] ) &&  ! empty( $parameter['value'] ) ) {
                    ...
                    $array[ sanitize_text_field( $parameter['name'] ) ] = $parameter['value'];
                }
            }
        }
    ...
    public function do_the_call() {
        ...
        if( is_wp_error( $this->response ) ) {
            return apply_filters( 'wpgetapi_raw_data', json_encode( $this->response ), $this );
        }
    ...
    public function build_request_args() {
        ...
        if( $this->method == 'POST' ) {
            ...
            if( isset( $this->body_parameters ) && $this->body_parameters != '' ) {

                foreach ($this->body_parameters as $key => $value ) {

                    // checking if the body parameters have a json string
                    if ( substr( $value, 0, 1 ) === '{' && substr( $value, -1 ) === '}' ) {
                        $this->body_parameters[ $key ] = json_decode( $value, true);
                    }
                ...
                if( $this->body_json_encode === true || $this->body_json_encode === 'true' )
                    $this->body_parameters = json_encode( $this->body_parameters );

                $body = apply_filters( 'wpgetapi_body_parameters', array( 
                    'body' => $this->body_parameters,
                ) );  

                $this->final_request_args = wp_parse_args( $body, $this->final_request_args );
            }

        }

        if( isset( $this->header_parameters ) && $this->header_parameters != '' ) {

            foreach ($this->header_parameters as $key => $value ) {

                // checking if the body parameters have a json string
                if ( substr( $value, 0, 1 ) === '{' && substr( $value, -1 ) === '}' ) {
                    $this->header_parameters[ $key ] = json_decode( $value, true);
                }

                // checking if the body parameters have an array within the string
                // if they do, create them as an actual array
                if ( substr( $value, 0, 1 ) === '[' && substr( $value, -1 ) === ']' ) {
                    $this->header_parameters[ $key ] = array( $value );
                }
    ...
    padding: 5px
            ...
            <div class="wpgetapi-debug">
                ...
                <div class="debug-item">
                    ...
                    <p class="small">The query string paramaters that are being sent in the API request.</p>
                    <div class="debug-result"><?php wpgetapi_pp( $this->query_parameters ); ?></div>
                </div>

                <div class="debug-item">
                    <h6>Headers</h6>
                    <p class="small">The headers that are being sent in the API request.</p>
                    <div class="debug-result"><?php wpgetapi_pp( $this->header_parameters ); ?></div>
                </div>

                <div class="debug-item">
                    <h6>Request Arguments</h6>
                    <p class="small">The request arguments sent in the API request.</p>
